...
# =============================================================================
# Cached helpers for expensive computations
# =============================================================================

@st.cache_data(show_spinner=False)
def _cached_detect_column_types(df_json: str) -> dict[str, list[str]]:
...
@st.cache_data(show_spinner=False)
def _cached_recommend_best_charts(df_json: str) -> list[dict[str, Any]]:
    """Cache chart recommendations."""
    df = pd.read_json(df_json)
    return recommend_best_charts(df)


@st.cache_data(show_spinner=False)
def _cached_generate_overview(df_json: str):
    """Cache overview tab charts."""
    df = pd.read_json(df_json)
    return generate_overview_charts(df)


@st.cache_data(show_spinner=False)
def _cached_generate_trends(df_json: str):
    """Cache trends tab charts."""
    df = pd.read_json(df_json)
    return generate_trends_charts(df)


@st.cache_data(show_spinner=False)
def _cached_generate_categories(df_json: str):
    """Cache categories tab charts."""
    df = pd.read_json(df_json)
    return generate_categories_charts(df)


@st.cache_data(show_spinner=False)
def _cached_generate_relationships(df_json: str):
    """Cache relationships tab charts."""
    df = pd.read_json(df_json)
    return generate_relationships_charts(df)


def _get_df_json(df: pd.DataFrame) -> str:
    """Serialize DataFrame to JSON string for caching."""
    return df.to_json()
...
def init_session_state() -> None:
    ...
    defaults = {
        ...
        "dashboard_overview_charts": None,
        "dashboard_trends_charts": None,
        "dashboard_categories_charts": None,
        "dashboard_relationships_charts": None,
        "dashboard_generated": False,
        "dashboard_df_json": None,
    }
...
def _process_upload(uploaded_file) -> None:
    ...
    try:
        ...
        st.session_state.dashboard_overview_charts = None
        st.session_state.dashboard_trends_charts = None
        st.session_state.dashboard_categories_charts = None
        st.session_state.dashboard_relationships_charts = None
        st.session_state.dashboard_generated = False
        st.session_state.dashboard_df_json = None
...
# =============================================================================
# Dashboard Analytics Center (Performance Optimized)
# =============================================================================


def render_dashboard_analytics_center() -> None:
    """Dashboard Analytics Center with lazy loading and performance optimization."""
    ...
    if is_large:
        st.warning(
            "⚡ Large dataset detected ({:,} rows). Dashboard is using optimized mode with sampling and limited charts.".format(len(df)),
            icon="⚡",
        )
    ...
    with col1:
        if st.button("Generate Dashboard", type="primary", use_container_width=True):
            with st.spinner("Building visualizations..."):
                try:
                    df_json = _get_df_json(df)
                    st.session_state.dashboard_recommendations = _cached_recommend_best_charts(df_json)
                    st.session_state.dashboard_df_json = df_json
                    st.session_state.dashboard_generated = True
                    # Pre-generate overview only; others load lazily per tab
                    st.session_state.dashboard_overview_charts = _cached_generate_overview(df_json)
                    st.session_state.dashboard_trends_charts = None
                    st.session_state.dashboard_categories_charts = None
                    st.session_state.dashboard_relationships_charts = None
                    log_action(
                        action="Generate Dashboard",
                        details="Generated dashboard visualizations (optimized mode).",
                        affected_rows=len(df),
                    )
    ...
    if st.session_state.get("dashboard_generated"):
...
def _render_overview_tab() -> None:
    ...
    charts = st.session_state.get("dashboard_overview_charts")
    if charts is None and st.session_state.get("dashboard_df_json"):
        with st.spinner("Loading overview charts..."):
            charts = _cached_generate_overview(st.session_state.dashboard_df_json)
            st.session_state.dashboard_overview_charts = charts
...
def _render_trends_tab() -> None:
    ...
    charts = st.session_state.get("dashboard_trends_charts")
    if charts is None and st.session_state.get("dashboard_df_json"):
        with st.spinner("Loading trends charts..."):
            charts = _cached_generate_trends(st.session_state.dashboard_df_json)
            st.session_state.dashboard_trends_charts = charts

    if charts:
        _render_chart_group(charts, max_per_row=2)

        # Show trend line charts from overview if available
        overview_charts = st.session_state.get("dashboard_overview_charts", [])
...
def _render_categories_tab() -> None:
    ...
    charts = st.session_state.get("dashboard_categories_charts")
    if charts is None and st.session_state.get("dashboard_df_json"):
        with st.spinner("Loading category charts..."):
            charts = _cached_generate_categories(st.session_state.dashboard_df_json)
            st.session_state.dashboard_categories_charts = charts
...
def _render_relationships_tab() -> None:
    ...
    charts = st.session_state.get("dashboard_relationships_charts")
    if charts is None and st.session_state.get("dashboard_df_json"):
        with st.spinner("Loading relationship charts..."):
            charts = _cached_generate_relationships(st.session_state.dashboard_df_json)
            st.session_state.dashboard_relationships_charts = charts

    if charts:
        heatmaps = [c for c in charts if c.chart_type == "heatmap"]
        if heatmaps:
...
def _render_advanced_analytics_tab() -> None:
    ...
    # Load all chart groups for advanced view
    df_json = st.session_state.get("dashboard_df_json")
    if df_json:
        all_charts = []
        # Ensure all groups are loaded
        for attr, loader in [
            ("dashboard_overview_charts", _cached_generate_overview),
            ("dashboard_trends_charts", _cached_generate_trends),
            ("dashboard_categories_charts", _cached_generate_categories),
            ("dashboard_relationships_charts", _cached_generate_relationships),
        ]:
            charts = st.session_state.get(attr)
            if charts is None:
                with st.spinner("Loading additional charts..."):
                    charts = loader(df_json)
                    st.session_state[attr] = charts
            if charts:
                all_charts.extend([c for c in charts if c.applicable])
